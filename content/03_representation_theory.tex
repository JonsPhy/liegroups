...
\defn[-]{Group Actions On Sets}{
    ...
    A \textbf{left action}\index{group action!left} of \(G\) on \(X\) is a map
    \[
    ...
    A \textbf{right action}\index{group action!right} of \(G\) on \(X\) is a map
    \[
...
\defn[-]{Orbit And Stabilizer}{
    ...
    The \textbf{orbit}\index{orbit} of \(x\), denoted by \(\operatorname{Orb}_G(x)\) or simply
    ...
    The \textbf{stabilizer}\index{stabilizer} of \(x\), denoted by \(\operatorname{Stab}_G(x)\) or
...
\ex{
        ...
        \]
        is the centralizer\index{centralizer} of \(g\).
...
\thmp{Orbit-Stabilizer Theorem}{\index{orbit-stabilizer theorem}
...
\ex{
    \begin{enumerate}[label=\textbf{\alph*)}]
        ...
        called the \textbf{order}\index{order (of a group element)} of \(g\), denoted
        \[
...
\defn[-]{Maps Of Representations}{
    ...
    A \textbf{map of representations}, or \textbf{intertwiner}\index{intertwiner}, is a linear map
    \[
        f : V \to V'
    \]
...
\ex{
    \begin{itemize}
        ...
        \textbf{trivial representation}\index{trivial representation} on \(V\), defined by
        \[
            \rho(g) = 1_V
            \qquad
            \forall g \in G.
        ...
        \textbf{regular representation}\index{regular representation} of \(G\) on \(\mathbb{K}[G]\):
        \[
            \rho(g)(h) := gh.
        \]
...
\defn[-]{Subrepresentations And Irreducibility}{
    ...
    A \textbf{subrepresentation}\index{subrepresentation} is a linear subspace \(W \leq V\) such that
    \[
        \rho(g)(w) \in W
        \qquad
    ...
    The representation \(V\) is \textbf{irreducible}\index{irreducible representation} if its only
...
\lemp{Schur's Lemma}{\index{Schur's lemma}
...
\defn[-]{Constructions From Old Representations}{
    ...
    \begin{itemize}
        \item The \textbf{direct sum representation}\index{direct sum representation} is
        \[
        ...
        \item The \textbf{tensor product representation}\index{tensor product representation} is
        \[
        ...
        \item The \textbf{dual representation}\index{dual representation} of \((V_1,\rho_1)\) is
        \[
            (V_1^*,\rho_1^*),
            \qquad
        ...
        \item The \textbf{Hom representation}\index{Hom representation} is the representation on
...
\defn[indecomposable representation]{Indecomposable Representation}{
...
\defn[semisimple representations]{Semisimple Representations}{
    Let \((V,\rho)\) be a representation of a group \(G\). We say that \(V\) is
    \textbf{semisimple}, or \textbf{completely reducible}\index{completely reducible}, if every
...
\ex{
    ...
    \]
    The tensor product decomposes according to the Clebsch-Gordan rule\index{Clebsch-Gordan rule}
    \[
        V_k \otimes V_\ell
        \cong
...
\defn[unitary representation]{Unitary Representation}{
...
\thmpr{Maschke's Theorem}{maschke}{\index{Maschke's theorem}
...
\defn[-]{Characters And Class Functions}{
    ...
    The \textbf{character}\index{character} of \(V\) is the function
    \[
        \chi_V : G \to \mathbb{C},
        \qquad
    ...
    A function \(f : G \to \mathbb{C}\) is called a \textbf{class function}\index{class function} if
...
\rmkb{
    ...
    \textbf{character table}\index{character table}.
}
...
\defn[invariant subspace]{Invariant Subspace}{
...
\defn[-]{Partitions And Young Diagrams}{
    A \textbf{partition}\index{partition} of \(n\) is a tuple
    \[
    ...
    A partition can be represented by a \textbf{Young diagram}\index{Young diagram}: draw
...
\defn[associative algebra]{Associative Algebras}{
...
\ex{
    ...
    \[
        1=e_{\mathrm{id}}.
    \]
    It is called the \textbf{group algebra}\index{group algebra} of \(G\).
}
...
\defn[Young symmetrizer]{Young Symmetrizers}{
...
\thmp{Frobenius Formula}{\index{Frobenius formula}
    ...
    See \citep{FultonRepresentationTheory2004}
}
...
\section{Applications of group theory in quantum mechanics}

\subsection{Symmetries of the Hamiltonian}

In quantum mechanics, the state space of a particle in \(\mathbb{R}^n\) is the
Hilbert space
\[
    \mathcal{H} = L^2(\mathbb{R}^n).
\]
Elements of \(\mathcal{H}\) are wave functions
\[
...
\[
    ...
    =
    \int \overline{\psi(x)}\, \varphi(x)\, dx.
\]

\defn[affine group]{Affine Group}{
    The \textbf{group of affine transformations} of \(\mathbb{R}^n\) is
    \[
        \mathrm{Aff}_n
        :=
        \left\{
            ...
            \,\middle|\,
            \alpha(x) = Ax + T,\ A \in \mathrm{GL}(n,\mathbb{R}),
            \ T \in \mathbb{R}^n
        \right\}.
    \]
    Identifying \(\alpha\) with the pair \((T, A)\) acting by
    \[
        (T, A) : x \mapsto Ax + T,
    \]
    composition reads
    ...
    and consequently
    \[
        \mathrm{Aff}_n \cong \mathbb{R}^n \rtimes \mathrm{GL}(n, \mathbb{R}).
    \]
}

\rmkb{
    There is a natural group homomorphism which remembers only the linear part:
    \[
        \mathrm{Aff}_n \to \mathrm{GL}(n,\mathbb{R}),
        \qquad
        (T, A) \mapsto A,
    \]
    The kernel consists of the transformations with \(A = \mathrm{id}\), namely
    \[
        x \mapsto x + T.
    \]
    Hence the translations form a normal subgroup of \(\mathrm{Aff}_n\).
}

In quantum mechanics, one is interested in transformations that preserve
distances, angles, and the Lebesgue measure. This restricts the linear part
from \(\mathrm{GL}(n,\mathbb{R})\) to \(O(n)\), and hence gives the subgroup
\[
    ...
    \left\{
        x \mapsto Ax + T
        \,\middle|\,
        A \in O(n),\ T \in \mathbb{R}^n
    \right\}.
\]

For \(\alpha(x) = Ax + T\), one has
\[
    \alpha^{-1}(x) = A^{-1}(x - T).
\]
The corresponding action on a wave function \(f\) is by pullback along the
inverse:
\[
    \rho(\alpha) f(x)
    =
    ...
    (f \circ \alpha^{-1})(x).
\]
For \(a, b \in \mathrm{Aff}_n\),
\[
    \rho(a)\rho(b) f(x)
    =
    (f \circ b^{-1} \circ a^{-1})(x)
    =
    (f \circ (ab)^{-1})(x)
    =
    \rho(ab) f(x).
...
\propp{
    The action of \(\mathbb{R}^n \rtimes O(n)\) on \(L^2(\mathbb{R}^n)\) is
    unitary.
}{
    Let \(\alpha(x) = Ax + T\) with \(A \in O(n)\), and substitute
    \[
        y = \alpha^{-1}(x),
        \qquad
        x = \alpha(y) = Ay + T.
    ...
    and therefore
    \[
        \langle \rho(\alpha) f \mid \rho(\alpha) g \rangle
        =
        \int \overline{f(\alpha^{-1}(x))}\, g(\alpha^{-1}(x))\, d^n x
        =
        \int \overline{f(y)}\, g(y)\, d^n y
        =
...
\defn{Hamiltonian}{
    A \textbf{Hamiltonian} is a self-adjoint, usually unbounded, operator
    \[
        H : \mathcal{H} \to \mathcal{H}.
    \]
}

\defn[spatial symmetries of the Hamiltonian]{Spatial Symmetries of the Hamiltonian}{
    The \textbf{group of spatial symmetries} of the Hamiltonian \(H\) is the
    subgroup
    \[
        \mathrm{Sym}(H)
        =
        \left\{
            a \in \mathbb{R}^n \rtimes O(n)
            \,\middle|\,
            [\rho(a), H] = 0
        \right\}.
    \]
...
\rmkb{
    Since \(\rho(a) H = H \rho(a)\) for every \(a \in \mathrm{Sym}(H)\), the
    Hamiltonian is an intertwiner of the representation of
    \(\mathrm{Sym}(H)\) on \(\mathcal{H}\) with itself.
}

Assume for the moment that the representation of \(\mathrm{Sym}(H)\) on
\(\mathcal{H}\) decomposes as
\[
    \mathcal{H}
    =
    \bigoplus_i N_i V_i,
\]
where the \(V_i\) are pairwise non-isomorphic irreducible representations and
\(N_i V_i\) is shorthand for the isotypic component
\(N_i \otimes V_i\), with \(N_i\) the multiplicity space. From Schur's lemma,
\[
    \langle \psi_i \mid H \varphi_j \rangle = 0
    \qquad
    \text{for}
    \qquad
    \psi_i \in N_i V_i,\quad
    \varphi_j \in N_j V_j,\quad
    i \neq j.
\]

\rmkb{
    Why does this work here? By the Peter-Weyl theorem, unitary
    representations of compact groups decompose into irreducible
    representations. Thus for compact symmetry groups, such as \(O(n)\) or
    \(SO(3)\), the direct-sum decomposition above is justified.

    If the symmetry group contains a non-compact part, such as translations
    \(\mathbb{R}^n\), the correct analogue is usually a direct integral rather
    than a direct sum. This is exactly what happens in Fourier theory and in
    the Bloch decomposition for lattice translations below.
}

It follows that \(H\) is block diagonal with respect to the above
decomposition. Moreover, Schur's lemma forces \(H\) to act as the identity on
the irreducible factor \(V_i\), hence
\[
    H = \bigoplus_i H_i \otimes I_{V_i}.
\]

\ex{
    \begin{itemize}
        \item \textbf{Free particle.} The Hamiltonian
        \[
            H = -\frac{\hbar^2}{2m}\nabla^2
        \]
        is invariant under translations and orthogonal transformations, so
        \[
            \mathrm{Sym}(H) = \mathbb{R}^n \rtimes O(n).
        \]

        \item \textbf{Particle in a potential.} For
        \[
            H = -\frac{\hbar^2}{2m}\nabla^2 + V(x),
        \]
        the multiplication operator \(V\) acts on wave functions by
        \[
            (Vf)(x) = V(x) f(x),
        \]
        so that, for \(a=\alpha \in \mathbb{R}^n \rtimes O(n)\),
        \[
            \rho(a)(Vf)(x)
            =
            V(\alpha^{-1}(x))\, f(\alpha^{-1}(x)),
        \]
        while
        \[
            V(\rho(a)f)(x)
        ...
        \[
            [\rho(a), V] f(x)
            =
            \bigl(V(\alpha^{-1}(x)) - V(x)\bigr)\, f(\alpha^{-1}(x)).
        \]
        Hence \(a\) is a symmetry if
        \[
            V(\alpha^{-1}(x)) = V(x)
            \qquad
        ...
        \[
            \mathrm{Sym}(H)
            =
            \{a \in \mathbb{R}^n \rtimes O(n) \mid
            V(a x) = V(x)\ \forall x \in \mathbb{R}^n\}.
        \]

        \item \textbf{Hydrogen atom.} The Coulomb potential
        \[
            V(x) \sim \frac{1}{\|x\|}
        \]
        is invariant under rotations, since
        \[
            \|Rx\| = \|x\|
            \qquad
            \forall R \in O(3).
        ...
        \[
            ...
            L^2(\mathbb{R}_{>0}, r^2\, dr) \otimes \mathcal{H}_\ell,
        \]
        where \(\mathcal{H}_\ell\) is the space of spherical harmonics of
        degree \(\ell\),
        \[
            \mathcal{H}_\ell
            =
            \operatorname{span}\{Y_\ell^m : m = -\ell, \dots, \ell\}.
        \]
        This is an irreducible representation of \(SO(3)\) of dimension
        \[
            \dim \mathcal{H}_\ell = 2\ell + 1.
        \]
        From Schur's lemma,
        \[
            \langle \psi_{\ell, m} | H | \varphi_{\ell', m'} \rangle = 0
            \qquad
            \text{if}\quad \ell' \neq \ell.
        \]
        Thus angular momentum labels the irreducible representations.

        \item \textbf{Lattices and Bloch decomposition.} Let
        \(\Delta \subseteq \mathbb{R}^n\) be a lattice, and consider a
        periodic potential
        \[
            V(x + T) = V(x)
            \qquad
            \forall T \in \Delta.
        \]
        Then \(H\) has at least the symmetry group \(\Delta\). Assuming
        \(\Delta \cong \mathbb{Z}^n\), choose a basis
        \(\{a_1, \dots, a_n\}\) of lattice vectors, so that every lattice
        vector can be written as
        \[
            T = m_1 a_1 + \cdots + m_n a_n,
            \qquad
            m_i \in \mathbb{Z}.
        \]
        Since \(\Delta \cong \mathbb{Z}^n\) is abelian, all of its irreducible
        representations are one-dimensional, i.e.\ group homomorphisms
        \[
            \mathbb{Z}^n \to U(1).
        \]
        The group of such homomorphisms is
        \[
            \mathrm{Hom}(\mathbb{Z}^n, U(1)) \cong {U(1)}^n.
        \]

        A convenient way to write these representations is by choosing plane
        waves: for \(k \in \mathbb{R}^n\), consider
        \[
            e^{2\pi i k \cdot x}.
        \]
        Then
        \[
            e^{2\pi i k \cdot (x - T)}
            =
            e^{2\pi i k \cdot x}\, e^{-2\pi i k \cdot T},
        \]
        so that the character of \(\Delta\) is
        \[
            \chi_k(T) = e^{-2\pi i k \cdot T}.
        \]
        In particular,
        \[
            \chi_k(a_j) = e^{-2\pi i k \cdot a_j},
        \]
        and the corresponding element of \({U(1)}^n\) is
        \[
            \bigl(e^{-2\pi i k \cdot a_1}, \dots,
            e^{-2\pi i k \cdot a_n}\bigr).
        \]

        Different values of \(k\) can yield the same character: \(k\) and
        \(k + G\) give the same character if and only if
        \[
            e^{-2\pi i G \cdot T} = 1
            \qquad
            \forall T \in \Delta,
        ...
        that is,
        \[
            G \cdot T \in \mathbb{Z}
            \qquad
            \forall T \in \Delta.
        \]

        \defn[reciprocal lattice and Brillouin zone]{Reciprocal Lattice and Brillouin Zone}{
            The \textbf{reciprocal lattice} is
            \[
                \Delta^*
                =
                \{G \in \mathbb{R}^n \mid
                G \cdot T \in \mathbb{Z}\ \forall T \in \Delta\}.
            \]
            Equivalently, if \(\{a_1, \dots, a_n\}\) is a basis of
            \(\Delta\), then the dual basis \(\{a_1^*, \dots, a_n^*\}\) is
            defined by
            \[
                a_i^* \cdot a_j = \delta_{ij},
            \]
            and the possible characters are labeled by
            \[
                k \in \mathbb{R}^n / \Delta^*.
            \]
            This quotient, or a fundamental domain for it, is called the
            \textbf{Brillouin zone}\index{Brillouin zone}.
        }

        Since the Hamiltonian commutes with lattice translations, the Hilbert
        space decomposes into sectors labeled by \(k\):
        \[
            \mathcal{H}
            =
            \int_{\mathrm{BZ}}^{\oplus} \mathcal{H}_k\, dk,
        \]
        where \(\mathcal{H}_k\) is the subspace transforming under
        translations as \(\chi_k\).
    \end{itemize}
}

\subsection{Selection rules}

In quantum mechanics, one considers both states
\[
    |\psi\rangle \in \mathcal{H}
\]
and operators on \(\mathcal{H}\). Observables are represented by Hermitian
operators \(O\), so that
\[
    \langle O\psi \mid \varphi \rangle
    =
    \langle \psi \mid O\varphi \rangle.
\]
In particular, expectation values
\[
    \langle \psi \mid O\psi \rangle
\]
are real. If \(O\) is positive, then
\[
    \langle \psi \mid O\psi \rangle \geq 0.
\]

Suppose \(G\) is a symmetry group of the system. Then \(\mathcal{H}\) carries
a unitary representation
\[
    \rho : G \to U(\mathcal{H}).
\]
The induced action on operators is by conjugation:
\[
    O
    \longmapsto
    \rho(g) O \rho(g^{-1}).
\]
For time evolution, the Heisenberg-picture operator is
\[
    O(t)
    =
    e^{-iHt} O(0) e^{iHt},
...
Let \(|\psi\rangle \in \mathcal{H}\). Then the transformed vector
\(O|\psi\rangle\) transforms as
\[
    O|\psi\rangle
    \longmapsto
    \rho(g)O\rho(g^{-1})\rho(g)|\psi\rangle
    =
    \rho(g)(O|\psi\rangle).
\]
Abstractly, this means that the evaluation map
\[
    \operatorname{End}(\mathcal{H}) \otimes \mathcal{H}
    \longrightarrow
    \mathcal{H},
    \qquad
    O \otimes |\psi\rangle
...
\propp{
    Suppose \(O\) transforms in an irreducible representation
    \(W \subseteq \operatorname{End}(\mathcal{H})\), and suppose
    \(|\psi_i\rangle\) and \(|\psi_j\rangle\) transform in irreducible
    representations \(V_i,V_j \subseteq \mathcal{H}\). If \(V_j\) does not
    ...
    then
    \[
        \langle \psi_j \mid O\psi_i \rangle = 0.
    \]
}{
    Since \(O\) transforms in \(W\) and \(|\psi_i\rangle\) transforms in
    \(V_i\), the vector \(O|\psi_i\rangle\) lies in the representation
    generated by \(W \otimes V_i\). If \(V_j\) is not one of the irreducible
    summands of \(W \otimes V_i\), then \(O|\psi_i\rangle\) has no component
    in the \(V_j\)-isotypic part of \(\mathcal{H}\). Orthogonality of distinct
    irreducible components gives
    \[
        \langle \psi_j \mid O\psi_i \rangle = 0.
    \]
}
...
\ex{
    Consider the ammonia molecule \(\mathrm{NH}_3\). Its spatial symmetry group
    is \(S_3\), permuting the three hydrogen atoms. The irreducible
    representations of \(S_3\) are
    \[
        \mathrm{Triv},
        \qquad
        \mathrm{Alt},
        ...
        \mathrm{Std}.
    \]
    Geometrically, the \(z\)-axis transforms trivially, while the \(x\)- and
    \(y\)-directions transform in the standard representation:
    \[
        \hat z \in \mathrm{Triv},
        \qquad
        \hat x,\hat y \in \mathrm{Std}.
    \]

    We apply the selection rule to the coordinate operator \(\hat y\), so
    \(W=\mathrm{Std}\). Let \(|\psi_i\rangle \in V_i\). Then
    \(\hat y|\psi_i\rangle\) transforms in the tensor product
    \[
        \mathrm{Std}\otimes V_i.
    \]
    Hence:
    \begin{itemize}
        \item If \(V_i=\mathrm{Triv}\), then
        \[
            \mathrm{Std}\otimes\mathrm{Triv}
            \cong
            \mathrm{Std}.
        \]
        Therefore
        \[
            \langle \psi_j \mid \hat y \mid \psi_i\rangle = 0
        \]
        unless \(|\psi_j\rangle\) transforms in \(\mathrm{Std}\).

        \item If \(V_i=\mathrm{Alt}\), then
        \[
            \mathrm{Std}\otimes\mathrm{Alt}
            \cong
            \mathrm{Std}.
        \]
        Again,
        ...
        \item If \(V_i=\mathrm{Std}\), then
        \[
            \mathrm{Std}\otimes\mathrm{Std}
            \cong
            \mathrm{Triv}\oplus\mathrm{Alt}\oplus\mathrm{Std}.
        \]
        Thus the matrix element
        \[
            \langle \psi_j \mid \hat y \mid \psi_i\rangle
        \]
        is not ruled out by symmetry.
    \end{itemize}
}

\ex{
    \textbf{Multi-particle states.}
    Suppose \(\mathcal{H}\) is the one-particle Hilbert space. For a system of
    \(N\) particles, the natural Hilbert space is
    \[
        \mathcal{H}^{\otimes N}.
    \]
    The symmetric group \(S_N\) acts on \(\mathcal{H}^{\otimes N}\) by
    permuting the tensor factors.

    In a quantum system of indistinguishable elementary particles, exchanging
    two particles can only change the state by a phase:
    \[
        e^{i\theta}|\psi\rangle
        =
        P_{(12\cdots N)}|\psi\rangle.
    \]
    Hence the multi-particle state lies in a one-dimensional representation of
    \(S_N\). There are only two such representations:
    \[
        \mathrm{Triv}
        \qquad\text{and}\qquad
        \mathrm{Alt}.
    \]
    The trivial representation gives bosons, while the alternating
    representation gives fermions. Thus
    \[
        \mathcal{H}_{\mathrm{bos}}
        =
        \operatorname{Sym}^N(\mathcal{H})
        \subseteq
        \mathcal{H}^{\otimes N},
        \qquad
        \mathcal{H}_{\mathrm{ferm}}
        =
        \bigwedge\nolimits^N \mathcal{H}
        \subseteq
        \mathcal{H}^{\otimes N}.
    \]
    If \(\dim \mathcal{H}=k\), then
    \[
    ...
    For a spin-\(\frac12\) system,
    \[
        \mathcal{H}
        =
        L^2(\mathbb{R}^3)\otimes\mathbb{C}^2,
    \]
    where \(L^2(\mathbb{R}^3)\) is the orbital part and
    \(\mathbb{C}^2=\operatorname{span}\{|\uparrow\rangle,|\downarrow\rangle\}\)
    is the spin part. Since spin-\(\frac12\) particles are fermions, the
    physical \(N\)-particle Hilbert space is
    \[
        \bigwedge\nolimits^N
        \bigl(L^2(\mathbb{R}^3)\otimes\mathbb{C}^2\bigr).
    \]

    For \(N=2\), the spin states decompose as
    \[
        (\mathbb{C}^2)^{\otimes 2}
        =
        \operatorname{Sym}^2\mathbb{C}^2
        \oplus
        \bigwedge\nolimits^2\mathbb{C}^2.
    \]
    The symmetric spin states are
    \[
    ...
    \]
    and the antisymmetric spin state is
    \[
        \frac{1}{\sqrt{2}}
        \bigl(|\uparrow\downarrow\rangle-|\downarrow\uparrow\rangle\bigr).
    \]
    ...
    \[
        \mathcal{H}
        =
        L^2(\mathbb{R}^3\times\mathbb{R}^3)
        \otimes
        \left(
            \operatorname{Sym}^2\mathbb{C}^2
            \oplus
        ...
        \right),
    \]
    but the physical subspace is still the fermionic subspace.

    For \(N=3\),
    \[
        \bigl(\mathbb{C}^2\bigr)^{\otimes 3}\cong \mathbb{C}^8.
    \]
    The fully symmetric spin states include
    \[
    ...
    and
    \[
        \operatorname{Sym}^3\mathbb{C}^2\cong\mathrm{Triv}^{\oplus 4}.
    \]
    On the other hand,
    \[
        \bigwedge\nolimits^3\mathbb{C}^2=0
    \]
    by the Pauli exclusion principle.

    To find the missing two-dimensional pieces, one uses the irreducible
    representations of \(S_3\). For example, for the Young diagram
    \[
        T=
        \ydiagram{2,1},
    \]
    the row group and column group are
    \[
        R_T=\{e,(12)\},
        \qquad
        C_T=\{e,(13)\}.
    \]
    Hence the Young symmetrizer is
    \[
        \gamma_T
        =
        a_T b_T
        =
        \left(\sum_{\sigma\in R_T}\sigma\right)
        \left(\sum_{\tau\in C_T}\operatorname{sgn}(\tau)\tau\right)
        =
        e+(12)-(13)-(132).
    \]
    The Specht module
    \[
        S_T:=\mathbb{C}[S_3]\gamma_T
    \]
    is the standard representation. Applying Gram-Schmidt to
    \(\gamma_T\) and \((13)\gamma_T\) gives a basis
    \[
        \gamma_T
        =
        \frac12\bigl(e+(12)-(13)-(132)\bigr),
    \]
    \[
        \gamma_T^\perp
        =
        \frac{1}{2\sqrt{3}}
        \bigl(-2e+(12)-(13)-(132)\bigr).
    \]
    Acting with these elements on
    \(|\uparrow\uparrow\downarrow\rangle\) gives states in the standard
    representation:
    \[
        \gamma_T|\uparrow\uparrow\downarrow\rangle
        =
        \frac{1}{\sqrt{6}}
        \bigl(
    ...
    \]
    \[
        \gamma_T^\perp|\uparrow\uparrow\downarrow\rangle
        =
        \frac{1}{\sqrt{2}}
        \bigl(
            |\downarrow\uparrow\uparrow\rangle
    ...
    \rmkb{
        Fermions and bosons only exhaust the possibilities in spacetime
        dimension at least \(3\). In two spatial dimensions, exchanges are
        represented by the braid group \(B_N\), not just by \(S_N\). Thus
        particles in two dimensions can be anyons.

        \begin{center}
        \begin{tikzpicture}[scale=0.9, line cap=round, line join=round]
            %% Braid generator: two crossings with alternating over/under (a twist)
            \node at (0.65,3.6) {\small braid generator};

            \coordinate (AT) at (0.1,3.1);
            \coordinate (BT) at (1.2,3.1);
            \coordinate (AB) at (0.1,0.1);
            \coordinate (BB) at (1.2,0.1);

            % Strand A in full (sinusoidal)
            \draw[very thick, domain=0:1, samples=80, smooth]
                plot ({0.1 + 0.7*sin(180*\x)}, {3.1 - 3*\x});

            % Strand B drawn on top with a white halo (B over A at both crossings)
            \draw[line width=5pt, white, domain=0:1, samples=80, smooth]
                plot ({1.2 - 0.7*sin(180*\x)}, {3.1 - 3*\x});
            \draw[very thick, domain=0:1, samples=80, smooth]
                plot ({1.2 - 0.7*sin(180*\x)}, {3.1 - 3*\x});

            % Overdraw strand A around the lower crossing (y \approx 0.96) so A is on top there
            \begin{scope}
                \clip (-0.2,0.55) rectangle (1.4,1.35);
                \draw[line width=5pt, white, domain=0:1, samples=80, smooth]
                    plot ({0.1 + 0.7*sin(180*\x)}, {3.1 - 3*\x});
                \draw[very thick, domain=0:1, samples=80, smooth]
                    plot ({0.1 + 0.7*sin(180*\x)}, {3.1 - 3*\x});
            ...
            %% Trivial braid
            \node at (4.0,3.6) {\small trivial braid};
            \draw[very thick] (3.55,3.1) -- (3.55,0.1);
            \draw[very thick] (4.45,3.1) -- (4.45,0.1);
            \foreach \p in {(3.55,3.1),(3.55,0.1),(4.45,3.1),(4.45,0.1)} {
                \fill \p circle (2pt);
            }
        \end{tikzpicture}
        \end{center}
